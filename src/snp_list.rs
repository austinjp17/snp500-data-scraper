...
async fn get_raw_snp() -> Result<String, reqwest::Error> {
    ...
    let page_data = helpers::make_request(&url, client).await?;
...
fn parse_snp(raw_page_data:&str )  -> DataFrame {
    ...
    for i in 0..comp_info.len() {
        ...
        match &mut comp_info[i] {
            TextPiece::Text { formatting: _, text } => {
    ...
    comp_info.retain(|variant| {
        ...
        let mut keep = true;
        if let TextPiece::Text { formatting: _, text } = variant {
            ...
            TextPiece::DoubleBraceExpression { tag: _, attributes } => {
                if let Some(first_attr) = attributes.get(0) {
                    if let Some(first_piece) = first_attr.value.pieces.get(0) {
                        match first_piece {
                            TextPiece::Text {formatting: _, text} => format!("{} {}", text, info),
                            _ => "OTHER".to_string()
                        }
                    } else {
                        "No pieces".to_string()
    ...
    match snp_df {
        Ok(df) => {snp_data = df}
        Err(e) => {println!("Error creating DF: {e}")}
    }
    snp_data
...
pub async fn get_snp_list() -> Result<DataFrame, reqwest::Error> {
    ...
    // println!("{:?}", snp_data[snp_data.len()-1]);
    Ok(snp_data)
}
